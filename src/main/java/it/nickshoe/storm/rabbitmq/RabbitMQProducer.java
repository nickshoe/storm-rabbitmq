...
import it.nickshoe.storm.rabbitmq.config.ConnectionConfig;
...
public class RabbitMQProducer implements Serializable {
	...
	private static final long serialVersionUID = -4893550590096622136L;
	...
	public void open(final Map<String, Object> stormConf) {
		this.logger = LoggerFactory.getLogger(RabbitMQProducer.class);
		
		this.connectionConfig = ConnectionConfig.getFromStormConfig(stormConf);
	...
	private Connection createConnection() throws IOException, TimeoutException {
		...
		if (connectionConfig.getHighAvailabilityHosts().isEmpty()) {
			connection = connectionFactory.newConnection();	
		} else {
			connection = connectionFactory.newConnection(connectionConfig.getHighAvailabilityHosts().toAddresses());
		}
