...
public abstract class TupleToMessage implements Serializable {
	...
	protected Message produceMessage(Tuple input) {
		return Message.forSending(
			...
			specifyMessageId(input)
		);
	}

	/**
	 * Extract message body as a byte array from the incoming tuple. This is
	 * required. This implementation must handle errors and should return null upon
	 * on unresolvable errors.
	 *
	 * @param input the incoming tuple
	 * @return message body as a byte array or null if extraction cannot be
	 *         performed
	 */
	protected abstract byte[] extractBody(Tuple input);

	/**
	 * Determine the exchange where the message is published to. This can be derived
	 * based on the incoming tuple or a fixed value.
	 *
	 * @param input the incoming tuple
	...
	/**
	 * Specify the headers to be sent along with this message. The default
	 * implementation return an empty map.
	 *
	 * @param input the incoming tuple
	...
	/**
	 * Specify message body content encoding. Default implementation skips the
	 * provision of this detail.
	 *
	 * @param input the incoming tuple
	...
	/**
	 * Specify whether each individual message should make use of message
	 * persistence when it's on a rabbitmq queue. This does imply queue durability
	 * or high availability or just avoidance of message loss. To accomplish that
	 * please read rabbitmq docs on High Availability, Publisher Confirms and Queue
	 * Durability in addition to having this return true. By default, message
	 * persistence returns false.
	 *
	 * @param input the incoming tuple
	 * @return whether the message should be persistent to disk or not. Defaults to
	 *         not.
	 */
	...
	/**
	 * Specify message-id property
	 * 
	 * @param input the incoming tuple
	 * @return the message id
	 */
	protected String specifyMessageId(Tuple input) {
		return null;
	}
}
